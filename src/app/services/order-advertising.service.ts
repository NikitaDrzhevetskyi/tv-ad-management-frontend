...
export class OrderAdvertisingService {
  ...
  createAdvertisingOrder(orderData: IAdvertisingOrder): Observable<any> {
    return this.http
      .post<{ message: string; advertisement: IAdvertisingOrder }>(
        this.apiUrl,
        orderData
      )
      .pipe(
  ...
  getAdvertisingOrders(ordersPerPage: number, currentPage: number): void {
    ...
    this.http
      .get<{
        message: string;
        advertisements: IAdvertisingOrder[];
  ...
  updateOrderStatus(orderId: string, status: string): Observable<any> {
    return this.http
      .put<{ message: string; advertisement: IAdvertisingOrder }>(
  ...
  getOrdersUpdateListener(): Observable<{
    orders: IAdvertisingOrder[];
    orderCount: number;
  }> {
  ...
  getAllApprovedAdvertisements(): Observable<{
    advertisements: IAdvertisingOrder[];
  }> {
    // Fetch all approved ads without pagination for revenue calculation
    const queryParams = '?status=approved&pagesize=1000'; // Large page size to get all
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          console.error(
            'Error fetching all approved advertising orders:',
            error
          );
          return throwError(error);
        })
